...
public class LoggerFactory {
	...
	static LogBackendFactory findLogBackendFactory() {
		// == BEGIN MODIFICATION: Attempt to load HighPerformanceFileLogBackendFactory directly ==
		try {
			Class<?> specificFactoryClass = Class.forName("com.j256.simplelogging.backend.HighPerformanceFileLogBackendFactory");
			LogBackendFactory highPerfFactory = (LogBackendFactory) specificFactoryClass.getDeclaredConstructor().newInstance();
			if (highPerfFactory.isAvailable()) {
				// Perform a quick check to ensure the backend can be created
				try {
					highPerfFactory.createLogBackend("HighPerfFactorySanityCheck");
					return highPerfFactory; // Successfully loaded and verified
				} catch (Throwable t) {
					// Backend creation failed, log for diagnostics and fall through.
					// System.err.println("LoggerFactory: HighPerformanceFileLogBackendFactory available but createLogBackend failed: " + t);
				}
			}
		} catch (ClassNotFoundException e) {
			// Factory class not found, this is not an error, just means it's not present or not yet compiled.
			// System.err.println("LoggerFactory: HighPerformanceFileLogBackendFactory not found, proceeding with standard discovery.");
		} catch (Exception e) {
			// Other exceptions during instantiation (e.g., linkage errors, security exceptions).
			// System.err.println("LoggerFactory: Error attempting to load HighPerformanceFileLogBackendFactory: " + e);
		}
		// == END MODIFICATION ==
